      ...
      <form onSubmit={handleSubmit} className="flex flex-col items-stretch justify-between flex-1 w-full gap-y-4">
        ...
        <fieldset>
          ...
          <label htmlFor="imgPicker" className='flex overflow-hidden transition-all border border-gray-300 rounded-md cursor-pointer hover:opacity-50 hover:brightness-95'>
            <input onChange={handlePicker} type="file" id='imgPicker' accept="image/png, image/jpeg" readOnly required className={productImage === DEFAULT_IMG ? '' : 'hidden'} />
            {
              productImage !== DEFAULT_IMG && <img src={productImage} alt="product image" />
            }
          </label>
        </fieldset>
        <fieldset className="flex flex-col gap-y-4">
          <input className="px-3 py-1 border border-gray-200 rounded-md outline-none focus-within:border-indigo-400" required type="text" name="product_name" placeholder="Product Name" />
          <textarea className="px-3 py-1 border border-gray-200 rounded-md outline-none focus-within:border-indigo-400" required name="description" placeholder="Product Description" cols={30} rows={10} />
